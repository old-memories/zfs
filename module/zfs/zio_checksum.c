    ...
    abd_t *abd, uint64_t size)
{
	...
	if (ci->ci_flags & ZCHECKSUM_FLAG_EMBEDDED) {
		 ...
		 */
		if (BP_GET_DEDUP(bp)) {
			htabd = abd_alloc_linear(HTDDT_HTSIZE(size), B_FALSE);
			
			abd_copy_off(htabd, abd, 0, 0, HTDDT_HTSIZE(size));
			ci->ci_func[0](htabd, HTDDT_HTSIZE(size), spa->spa_cksum_tmpls[checksum], &h_cksum);
			zio->blk_h_cksum = h_cksum;

			abd_copy_off(htabd, abd, 0, size - HTDDT_HTSIZE(size), HTDDT_HTSIZE(size));
			ci->ci_func[0](htabd, HTDDT_HTSIZE(size), spa->spa_cksum_tmpls[checksum], &t_cksum);
